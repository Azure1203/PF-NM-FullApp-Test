        ...
        <Tooltip>
          ...
          <TooltipContent>
            {backupStatus?.nextRun ? (
              <p className="text-xs">Next auto-backup: {new Date(backupStatus.nextRun).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
            ) : (
              <p className="text-xs">Export all orders to Google Sheets</p>
            )}
          </TooltipContent>
        </Tooltip>
      </div>
