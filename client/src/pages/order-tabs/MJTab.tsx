...
export function MJTab({ orderId }: { orderId: number }) {
  const { data, isLoading } = useQuery<MJData>({
    queryKey: ['/api/orders', orderId, 'data', 'mj'],
    queryFn: () => fetch(`/api/orders/${orderId}/data/mj`, { credentials: 'include' }).then(r => r.json()),
    enabled: !!orderId,
  });

  if (isLoading) return <div className="space-y-3">{[0, 1, 2].map(i => <Skeleton key={i} className="h-8 w-full" />)}</div>;
  if (!data || data.items.length === 0) {
    return <div className="text-center py-8 text-muted-foreground text-sm">No M&amp;J Doors items in this order.</div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <a href={`/api/orders/${orderId}/export/mj`} target="_blank" rel="noreferrer">
            <Button variant="outline" size="sm" data-testid="button-mj-csv">
              <Download className="w-4 h-4 mr-1.5" />
              Download CSV
            </Button>
          </a>
          <a href={`/api/orders/${orderId}/pdf/mj`} target="_blank" rel="noreferrer">
            <Button variant="outline" size="sm" data-testid="button-mj-pdf-tab">
