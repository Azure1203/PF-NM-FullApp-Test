...
// Processed emails deduplication table - originally for Outlook, now used exclusively by AgentMail.
// Kept under the legacy DB table name to avoid a DB migration.
...
export type ProcessedEmail = typeof processedOutlookEmails.$inferSelect;
...
export const allmoxyProducts = pgTable("allmoxy_products", {
  ...
  pricingProxyId: integer("pricing_proxy_id").references(() => proxyVariables.id, { onDelete: 'set null' }),
  exportProxyId: integer("export_proxy_id").references(() => proxyVariables.id, { onDelete: 'set null' }),
...
export const insertAttributeGridSchema = createInsertSchema(attributeGrids).omit({ id: true });
export type AttributeGrid = typeof attributeGrids.$inferSelect;
...
export const insertAttributeGridRowSchema = createInsertSchema(attributeGridRows).omit({ id: true });
export type AttributeGridRow = typeof attributeGridRows.$inferSelect;
