...
def make_styles():
    return {
        ...
        'title':    ParagraphStyle('TT',  fontName='Helvetica-Bold', fontSize=16, leading=20, alignment=TA_CENTER),
...
def build_header(data, styles):
    ...
    flowables = []
    flowables.append(Paragraph("NETLEY 5 PIECE SHAKER JOB LIST", styles['title']))
...
def build_drawer_front_section(section, styles):
    return _build_standard_section(section, styles, 'Drawer Fronts')


def build_door_section(section, styles):
    return _build_standard_section(section, styles, 'Doors')


def _build_standard_section(section, styles, type_header):
    P  = lambda txt, st='normal': Paragraph(txt, styles[st])
    PB = lambda txt: Paragraph(txt, styles['bold'])
    ...
    checkmarks  = section.get('supplierCheckmarks', 1)

    flowables = []

    checkmarks_str = '  '.join(['X'] * checkmarks)
    supplier_row = Table(
        [[PB("Supplier:"), P("Premoule"), PB(""), P(checkmarks_str)]],
    ...
    COLS = ['ID', 'Qty', 'Height', 'Width', 'Thickness', type_header]
    col_w_defs = {
        'ID':             USABLE_W * 0.07,
        'Qty':            USABLE_W * 0.06,
        'Height':         USABLE_W * 0.14,
        'Width':          USABLE_W * 0.14,
        'Thickness':      USABLE_W * 0.14,
        type_header:      USABLE_W * 0.45,
    }
    ...
    for item in items:
        row = [
            ...
            Paragraph(fmt_mm(item.get('thickness')),   styles['cell']),
            Paragraph(str(item.get('type', '')),       styles['cell_l']),
        ]
...
def build_glass_section(section, styles):
    P  = lambda txt, st='normal': Paragraph(txt, styles[st])
    PB = lambda txt: Paragraph(txt, styles['bold'])
    ...
    COLS = ['ID', 'Qty', 'Height', 'Width', 'Thickness', 'Glass', 'Buyout Or Stock?']
    col_w_defs = {
        'ID':                USABLE_W * 0.07,
        'Qty':               USABLE_W * 0.06,
        'Height':            USABLE_W * 0.12,
        'Width':             USABLE_W * 0.12,
        'Thickness':         USABLE_W * 0.12,
        'Glass':             USABLE_W * 0.28,
        'Buyout Or Stock?':  USABLE_W * 0.23,
    }
    col_ws = [col_w_defs[c] for c in COLS]
...
def generate(data):
    ...
    doc = BaseDocTemplate(
        ...
        title=f"NETLEY 5 PIECE SHAKER JOB LIST - Order #{order_id}",
    )
