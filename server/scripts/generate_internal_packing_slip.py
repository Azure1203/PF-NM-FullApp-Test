...
def make_styles():
    ...
    return {
        ...
        'warning':   ParagraphStyle('WN',  fontName='Helvetica-Bold',  fontSize=11,   leading=14, alignment=TA_CENTER, textColor=RED),
...
def build_header(data, styles):
    ...
    flowables = []
    flowables.append(Paragraph("INTERNAL PACKING LIST", styles['title']))
...
def cell_value(col, item):
    ...
    mapping = {
        ...
        'Buyout Or Stock?': str(item.get('supplyType', '')),
        'Rack Location':    str(item.get('rackLocation', '')),
    }
...
def generate(data):
    ...
    doc = BaseDocTemplate(
        ...
        title=f"Internal Packing List #{order_id}",
    )
