    ...
    for (const message of messages) {
      ...
      for (const attachment of message.attachments) {
        ...
        try {
          ...
          if (matchingFile) {
            log(`Fetching download URL for "${filename}" (attachmentId: ${attachment.attachment_id})...`, 'agentmail-scheduler');
            const attachmentDetail = await getAgentMailAttachment(message.id, attachment.attachment_id);
            log(`Got download_url for "${filename}": ${attachmentDetail.download_url.substring(0, 60)}...`, 'agentmail-scheduler');
